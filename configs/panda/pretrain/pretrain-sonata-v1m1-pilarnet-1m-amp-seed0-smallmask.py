...
model = dict(
    ...
    up_cast_level=2,
    # geo_adversary_weight=0.1,
    # geo_adversary_weight_warmup_ratio=0.05,
)

# scheduler settings
epoch = 10 * (1_000_000 // num_events)
wandb_run_name = f"sonata-XL-pilarnet-pretrain-{num_gpus}GPU-{num_events}ev-amp-seed{seed}-NormCoords-logE-{epoch}epoch-some-aug-noXYZ"  # Descriptive name for this run
...
transform = [
    ...
    dict(
        ...
        # center_sampling='cnms', # or cnms
        # center_sampling_kwargs=dict(radius=20/(768 * 3 ** 0.5 / 2), overlap_factor=0.73, K=512),
        ...
        global_shared_transform=[
            ...
            # dict(type="ElasticDistortion", distortion_params=[[8 * grid_size, 4 * grid_size]]),
        ],
        global_transform=[
            dict(type="CenterShift", apply_z=False, axes=("y", "z")),
            ...
            dict(type="RandomFlip", p=0.5, axes=("y", "z")),
            dict(
                type="RandomJitter",
                sigma=grid_size / 4,
                clip=grid_size,
            ...
            # dict(type="MultiplicativeRandomJitter", sigma=0.05, clip=0.05, keys=("energy"), p=0.8),
            # di[str, str | bool | tuple[Literal['y'], Literal['z']]]ct(type="ElasticDistortion", distortion_params=[[128 * grid_size, 32 * grid_size]]),
        ],
        local_transform=[
            dict(type="CenterShift", apply_z=False, axes=("y", "z")),
            # dict(type="RandomRotate", angle=[-1, 1], axis="z", center=[0, 0, 0], p=0.8),
            ...
            #     p=0.2,
            # ),
            # dict(type="ElasticDistortion", distortion_params=[[128 * grid_size, 32 * grid_size]]),
        ],
        max_size=30000,
    ),
    dict(type="ToTensor"),
    ...
    dict(
        ...
        global_feat_keys=("global_energy",),
        local_feat_keys=("local_energy",),
    ),
]
